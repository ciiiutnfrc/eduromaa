...
void iniTacometros(void)
{
  ...
  gpioWrite(TACOMETRO_GPIO_H, TACOMETRO_OFF);
	...
	Chip_TIMER_TIMER_SetCountClockSrc(LPC_TIMER0, TIMER_CAPSRC_RISING_PCLK, TACOMETRO_CAP_IZQ);
...
void TIMER0_IRQHandler(void)
{
	...
	if (Chip_TIMER_CapturePending(LPC_TIMER0, TACOMETRO_CAP_IZQ )) {
		...
		if (tacometro_izq.t_flanco_previo == 0){
			tacometro_izq.t_flanco_previo = Chip_TIMER_ReadCapture(LPC_TIMER0, TACOMETRO_CAP_IZQ);
			tacometro_izq.cuenta = 0;

		} else {

			tacometro_izq.t_flanco_actual = Chip_TIMER_ReadCapture(LPC_TIMER0, TACOMETRO_CAP_IZQ);
			...
			if(tacometro_izq.t_flanco_actual < tacometro_izq.t_flanco_previo){
				...
				tacometro_izq.t_flancos = tacometro_izq.t_flanco_actual + (~tacometro_izq.t_flanco_previo) + 1;
			} else {
				tacometro_izq.t_flancos = tacometro_izq.t_flanco_actual - tacometro_izq.t_flanco_previo;
			}
	...
	if (Chip_TIMER_CapturePending(LPC_TIMER0, TACOMETRO_CAP_DER )) {
		...
		if (tacometro_der.t_flanco_previo == 0){
			tacometro_der.t_flanco_previo = Chip_TIMER_ReadCapture(LPC_TIMER0, TACOMETRO_CAP_DER);
			tacometro_der.cuenta = 0;
		} else {
			tacometro_der.t_flanco_actual = Chip_TIMER_ReadCapture(LPC_TIMER0, TACOMETRO_CAP_DER);
			...
			if(tacometro_der.t_flanco_actual < tacometro_der.t_flanco_previo){
				...
				// (se obtiene haciendo el complemento a 2) mas el flanco actual
				tacometro_der.t_flancos = tacometro_der.t_flanco_actual + (~tacometro_der.t_flanco_previo) + 1;
			} else {
				tacometro_der.t_flancos = tacometro_der.t_flanco_actual - tacometro_der.t_flanco_previo;
			}
